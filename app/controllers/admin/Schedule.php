...
class Schedule extends Admin_Controller 
{
    ...
    */
    ...
    function index()
    {
        ...
        $this->getLimit();
        $this->_total = count($this->schedule_model->list()->result());
        $collection = $this->schedule_model->list($condition, $this->limit)->result();
        $collectionData = $this->schedule_model->parseData($collection);
    	...
    	$this->outputData = [
			...
			'subPage'=> 'schedule',
			'list' => $collectionData,
            'delAll' => admin_url('schedule/delListChoose'),
            'addNew' => admin_url('schedule/updated/0'),
            'pagination' => $pagination
		];

	    $this->render('admin/schedule/list');
    }
   
    function updated($id)
    {	
        $schedule =  $this->schedule_model->getDetailData(['id' => $id]);
	    if ($id) {
            $pageTitle = 'Chỉnh sửa';
        ...
        $this->outputData = [
            ...
            'action' => $action,
            'option' => [],
            'linkSave' => admin_url('schedule/save')
        ];
	    
		$this->render('admin/schedule/update');
    }
    ...
    function updateStatus()
    {
    	...
    	//check product exist
    	if ($this->schedule_model->check_exists(array('id' => $id))) {
    		// click ok status
	    	if ($_POST['active'] == 1) {
	    		$this->schedule_model->updateData(array('id' => $id), array($action => 0));
                $resp = ['result' => 'glyphicon-move', 'num' => 0];
	    	} else {
	    		$this->schedule_model->updateData(array('id' => $id), array($action => 1));
    ...
    function delListChoose()
    {
    	...
    	if (count($list_id) == 1) {
    		$this->schedule_model->deleteData($list_id);
    	} else {
        ...
        echo json_encode(array('result' => admin_url('schedule'))); exit(); 
    }
    ...
    function getPagination()
    {
        ...
        $limit = $this->limit;
        $perPage = $limit[0];
        $url = './admin/schedule'.$this->queryString;

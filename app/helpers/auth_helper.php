 ...
 */
	function isAdmin()
	{
		...
		return $CI->session->userdata('login_admin') ? TRUE: FALSE;
	}
	
	function isMember()
	{
		$CI =& get_instance();
		return ( count($CI->session->userdata('CI_login')) > 1 ) ? true : false;
	}

	function manageStoreAddress()
	{
		$CI =& get_instance();
		if ($CI->session->userdata('login_admin')) {
			return $CI->session->userdata('address_id');
		}

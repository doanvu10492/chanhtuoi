<?php
 class Category_posts_model extends My_Model { 

    public $table = TB_CGR_POSTS;
    public $key = 'id_cate';
    public $category = TB_CGR_POSTS;
    public $tableModule = TB_MODULE;

    public function __construct() 
    {
    	$this->table = TB_CGR_POSTS;
    ...
    public function listCategory($condition = array(), $limit = array(), $order_by = NULL)
    {
        ...
        $this->db->join(
            "{$this->tableModule}",
            "{$this->tableModule}.id = {$this->table}.module", 
            "left"
        );
        ...
        $this->db->select("
            ...
            {$this->table}.isHome,
            {$this->tableModule}.name as module_name
        ");
    ...
    public function parseCategoryData($data)
    {
    	...
    	foreach ($data as $row) {
    		$count++;
    		$row->count = $count;
    		$row->link_update = admin_url('category_posts/updated/'.$row->id);
    		$row->link_delete = admin_url('category_posts/delete/'.$row->id);
    		$row->link_active = admin_url('category_posts/updateStatus/'.$row->id);
            $row->link_footer = admin_url('category_posts/updateStatus/'.$row->id);
            $row->link_menu = admin_url('category_posts/updateStatus/'.$row->id);
